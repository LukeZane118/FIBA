...
from utils.show_example import generate_rows, show_images
...
class Helper:
    def __init__(self, current_time, params, name):
        ...
        self.folder_path = f'saved_models/model_{self.name}_' + ((params['postfix'] + '_') if len(params['postfix']) > 0 else '') + f'{current_time}'
        try:
        ...
        if self.params['use_tensorboard']:
            ...
            logger.info("Got writter for tensorboard.")
    ...
    @staticmethod
    def gradient_sim(model, loss_poi, loss_clean, optim):
        grads_poi = []
        loss_poi.backward(retain_graph=True)
        for param in model.parameters():
            if param.grad is not None:  # Check if gradients are available
                grads_poi.append(copy.deepcopy(param.grad.view(-1)))  # Flatten the gradients and append to the list
        
        grads_clean = []
        loss_clean.backward(retain_graph=True)
        for param in model.parameters():
            if param.grad is not None:  # Check if gradients are available
                grads_clean.append(param.grad.view(-1))  # Flatten the gradients and append to the list
        
        layer_gradient_similarity = sum(F.cosine_similarity(p, c - p, dim=0).item() for p, c in zip(grads_poi, grads_clean)) / len(grads_poi)
        
        grads_poi = torch.cat(grads_poi)
        grads_clean = torch.cat(grads_clean) - grads_poi
        
        gradient_similarity = F.cosine_similarity(grads_poi, grads_clean, dim=0).item()
        optim.zero_grad(set_to_none=True)
        # model.zero_grad(set_to_none=True)
        return gradient_similarity, layer_gradient_similarity
    ...
    def accumulate_weight(self, weight_accumulator, epochs_submit_update_dict, state_keys, num_samples_dict, adversarial_name_keys):
        """
         return Args:
    ...
    def foolsgold_update(self,target_model,updates):
        ...
        for i, name in enumerate(names):
            if name in self.params['adversary_list']:
    ...
    def save_trigger(self, epoch, example_images=None):
        if self.params['save_trigger'] and epoch in self.params['save_on_epochs']:
            dir_path = os.path.join(self.folder_path, f"epoch_{epoch}")
            logger.info(f'Saving trigger on epoch {epoch}')
            try:
                model = self.target_model
                for idx, trigger in enumerate(self.triggers):
                    trigger_name = '{0}/trigger_{1}_last.pt.tar'.format(self.params['folder_path'], idx)
                    # self.save_checkpoint(trigger.state_dict(), False, filename=f'{trigger_name}.epoch_{epoch}')
                    torch.save(trigger.state_dict(), trigger_name)
                    pattern = np.clip(trigger.get_pattern().cpu().detach().numpy().transpose((1, 2, 0)), 0, 1)
                    mp.image.imsave(os.path.join(self.params['folder_path'], f"trigger{idx}_img.png"), pattern)
                    if example_images is not None:
                        path = os.path.join(dir_path, f"trigger_{idx}")
                        if not os.path.exists(path):
                            os.makedirs(path)
                        images, labels = example_images
                        rows, titles = generate_rows(model, trigger, self.params['target_layers'], images, labels, self.params['poison_label_swap'], torch.device("cuda"))
                        # print(f'Save examples for trigger {idx}.')
                        show_images(rows, [[f"{s}\n{t}" for s, t in zip(['Clean', 'Poisoned', 'Clean', 'Poisoned'], titles[0])]] + titles[1:], save_path=path)
            except Exception as e:
                logger.info(f'Failed saving trigger {idx}.')
                logger.info(e)
    ...
    def save_file(self, file_name=None, data_list=None, folder_name=None):
