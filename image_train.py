...
from utils.utils import apply_grad_mask


def ImageTrain(helper: ImageHelper, start_epoch, local_model, target_model, is_poison, agent_name_keys):
    ...
    for model_id in range(helper.params['no_models']):
        ...
        last_local_model = copy.deepcopy(target_model.state_dict())
        client_grad = [] # only works for aggr_epoch_interval=1

        agent_name_key = agent_name_keys[model_id]
        is_adversary = agent_name_key in adversary_list
        ## Synchronize LR and models
        model: SimpleNet = local_model
        model.copy_params(target_model.state_dict())
        
        model.train()
        adversarial_index = -1
        localmodel_poison_epochs = set(helper.params['poison_epochs'])
        if is_poison and is_adversary:
            adversarial_index = adversary_list.index(agent_name_key)
            localmodel_poison_epochs = set(helper.params[f'{adversarial_index}_poison_epochs'])
            main.logger.info(
                f'poison local model {agent_name_key} index {adversarial_index} ')
                    # break
            if len(adversary_list) == 1:
                adversarial_index = -1  # the global pattern
            if helper.params['gradmask_ratio'] != 1. and start_epoch in localmodel_poison_epochs:
                mask_grad_list = helper.grad_mask(model, [helper.params['adversary_list'][adversarial_index]], nn.CrossEntropyLoss())
            else:
                mask_grad_list = None
        ...
        for epoch in range(start_epoch, start_epoch + helper.params['aggr_epoch_interval']):
            ...
            for name, _ in target_model.named_parameters():
                target_params_variables[name] = last_local_model[name].clone().detach()
            is_in_poison_epochs = epoch in localmodel_poison_epochs
            if is_poison and is_adversary and is_in_poison_epochs:
                ...
                helper.local_train_trigger(adversarial_index)
                ...
                for internal_epoch in range(1, internal_epoch_num + 1):
                    ...
                    for batch_id, batch in enumerate(data_iterator):
                        ...
                        if not helper.params["poi_clean_alt_train"]:
                            # DBA way
                            ...
                            # our way: calculate loss separately
                            output_poi, output_cle = output[:poison_num], output[poison_num:]
                            targets_poi, targets_cle = targets[:poison_num], targets[poison_num:]
                            clean_loss = nn.functional.cross_entropy(output_cle, targets_cle)
                            poi_loss = criterion(output_poi, targets_poi)
                        
                            if clean_loss < 0.1:
                                clean_loss_ = clean_loss * 0.5
                                poi_loss_ = poi_loss * 0.5
                            else:
                                clean_loss_ = clean_loss
                                poi_loss_ = poi_loss * 0.
                            
                            class_loss = clean_loss_ + poi_loss_
                            
                            # version 2
                            loss += helper.params['alpha_loss'] * class_loss + \
                                (1 - helper.params['alpha_loss']) * distance_loss
                            poi_loss_ = helper.params['alpha_loss'] * poi_loss_ + \
                                (1 - helper.params['alpha_loss']) * distance_loss
                            poi_loss_.backward(retain_graph=True)
                            if helper.params['gradmask_ratio'] != 1.:
                                apply_grad_mask(model, mask_grad_list)
                            clean_loss_ = helper.params['alpha_loss'] * clean_loss_ 
                            clean_loss_.backward()
                        ...
                        # our way
                        if helper.params["poi_clean_alt_train"]:
                            total_clean_loss += clean_loss.data
                            total_poi_loss += poi_loss.data
                    ...
                    total_l = total_loss / (batch_id + 1)
                    main.logger.info(
                        '___PoisonTrain {} ,  epoch {:3d}, local model {}, internal_epoch {:3d},  Average loss: {:.4f}, '
                        'Accuracy: {}/{} ({:.4f}%), train_poison_data_count: {}, ssim: {:.4f}'.format(model.name, epoch, agent_name_key,
                                                                                      internal_epoch,
                                                                                      total_l, correct, dataset_size,
                                                                                     acc, poison_data_count, -ssim / (batch_id + 1))
                        )
                    if helper.params["poi_clean_alt_train"]:
                        main.logger.info(f"Average clean loss: {total_clean_loss / (batch_id + 1):.4f}, average poi loss: {total_poi_loss / (batch_id + 1):.4f}")
                ...
                if not helper.params['baseline']:
                    ...
                    if helper.params['vis_trigger_split_test']:
                        model.trigger_agent_test_vis(vis=main.vis, epoch=epoch, acc=epoch_acc, loss=epoch_loss,
                                                     eid=helper.params['environment_name'],
                                                     name=str(agent_name_key)  + "_combined")
                    ...
                    main.logger.info('___Test {}&Local trigger{} poisoned: {}, epoch: {}: Average loss: {:.4f}, '
                    'Accuracy: {}/{} ({:.4f}%)'.format(model.name, agent_name_key, True, epoch,
                                                        epoch_loss, epoch_corret, epoch_total, epoch_acc))
            ...
            if is_poison and is_adversary and not is_in_poison_epochs:
                # test on local triggers
                epoch_loss, epoch_acc, epoch_corret, epoch_total = \
                    test.Mytest_poison_agent_trigger(helper=helper, model=model, agent_name_key=agent_name_key)
                csv_record.poisontriggertest_result.append(
